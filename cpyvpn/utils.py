...
def do_https_request(url, data=None, headers={}, with_cookies=False, rawval=False, filter_data=False):
    ...
    ret = rdata if rawval else rdata.decode("utf8")
...
def do_ccc_request(url, data=None, headers={}, with_cookies=False, rawval=False, rc_check=True):
    ...
    if not rh:
        raise Exception("No ResponseHeader!")
    if rc_check and int(rh.get("return_code", -1)) != 600:
        raise Exception("Bad return_code!")

    rd = ret.find("ResponseData")
    if rd is None:
        raise Exception("No ResponseData!")
    return ret
...
def add_common_client_args(parser):
    ...
    parser.add_argument("-p", "--passwd", default="", type=str, help="Login (password).")
