...
class AuthHelper(object):
    ...
    def cert_login(self):
        body = CPRR({"CCCclientRequest":
              ...
              "RequestData":{
                  "client_type":self.ct,
                  "selectedLoginOption":self.realm
                  }
              }
              }).serialize()[:-1]
...
class SNXAuth:
    ...
    def init(self):
        ...
        def ma_login():
            ...
            opts.password = self.opts.passwd
        ...
        if use_ma:
            ...
            self.auth_obj = AuthHelper(self.opts.ct, realm=realm, login=login, pwd=pwd, cert_path=self.cert_url, url=self.opts.url, ui=self.ui)
